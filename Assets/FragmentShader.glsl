...
const light lights[] = light[4](
	...
	// green light
	light(vec3(-3.0, 1.0, 5.0), vec3(0.0, 1.0, 0.0), 10.0, 1.0),
...
vec4 trace(vec3 origin, vec3 dirEyeToTriangle)
{
	...
	if (intersectTriangles(origin, dirEyeToTriangle, eyeHitTriangle))
	{
		...
		for(int j = 0; j < 4; j++)
		{
			...
			vec3 reflection = addReflectionToPixColor(lights[j], dirEyeToTriangle, eyeHitTriangle, 4);
